...
function checkUser(username,email,callback) {
	...
	var XHR=createXHR();
	XHR.open("POST","/DragonAcademyWebsiteV2/php/checkUser.php",false);
...
function checkForm(e) {
	...
	with(theform) 
	{
		if(checkEmpty(firstname.value)) 
			{ forenameFb.innerHTML="Please add your forename/s"; iscomplete=false;}
		else
		...
		if(checkAge(yearob.value,monthob.value,dayob.value))
			{ ageFb.innerHTML="Sorry, you must be 4 or over to register."; iscomplete=false;}
		else
			{ ageFb.innerHTML=""; }
