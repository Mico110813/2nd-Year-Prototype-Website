...
<body>
	...
	<section>
		...
		<?php
			$db=createConnection();
			$drlevsql="select studentinfo.username, stuelement.elementid, elementlevel.elemname, stuelement.elemlevel
						...
						and (studentinfo.username = '$username' );";
			$drlev = $db->prepare($drlevsql);
			$drlev->execute();
			$drlev->store_result();
			$drlev->bind_result($username,$elementid, $elemname, $elemlevel);
			if($drlev->num_rows>0) 
				{
					while($drlev->fetch())
					{
						?>
						<div class = "levels"><?php
							echo "$elemname";?><br><?php echo "$elemlevel";?><br>
							<img src="assets/<?php echo $elemname; ?>.png" width="200" height="200"><br>
						</div>
						<?php
						}
					?>
