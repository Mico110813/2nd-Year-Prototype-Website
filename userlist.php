	...
	<section>
		...
		<?php
			...
			if($currentuser['userlevel']<3)
				{
					$userlistsql="select clientid, username from studentinfo where username = '$username';";
				}
			else
				{
					$userlistsql="select clientid, username from studentinfo;";
				}
			$userlist = $db->prepare($userlistsql);
			$userlist->execute();
			$userlist->store_result();
			$userlist->bind_result($clientid,$username);
			if($userlist->num_rows>0) 
				{?>
					<form id="listusers" name="listusers" method="post" action="edituser.php">
						<fieldset>
							<legend>Edit User</legend>
								<label for="clientid">Select User to Edit </label>
								<select name="clientid" id="clientid" required>
									<?php
										while($userlist->fetch())
										{
											echo "<option value='$clientid'>$username</option>";
										}
									?>
								</select>
